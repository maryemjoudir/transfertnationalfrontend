...
const IndexAgents = () => {
  ...
  const filteredAgents = currentAgents.filter((agent) =>
    `${agent[searchCriteria]}`.toLowerCase().includes(searchTerm.toLowerCase())
  );
                              ...
                              <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                ...
                                <div>
                                  <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                    ...
                                    {[...Array(Math.min(3, Math.ceil(agents.length / ITEMS_PER_PAGE))).keys()].map((number) => (
                                      <button
                                        key={number + 1}
                                    ...
                                    <button
                                      ...
                                      className={`${currentPage === Math.ceil(agents.length / ITEMS_PER_PAGE)
                                        ? 'pointer-events-none opacity-50 bg-white border-gray-300 text-gray-500 hover:bg-gray-50'
                                        : 'bg-white border-gray-300 text-gray-500 hover:bg-gray-50 cursor-pointer'
                                        } relative inline-flex items-center px-4 py-2 border text-sm font-medium`}
                                      style={{ borderRadius: '4px' }}
