...
const UpdateAgent = () => {
  ...
  useEffect(() => {
    const fetchClientData = async () => {
      try {
        ...
        setFormData({
          code: response.data.nidentity,
          nom: response.data.name,
          email: response.data.email,
          password: "",
          ville: response.data.ville,
          adresse: response.data.adresseLegale,
          solde: response.data.montant,
          ngsm: response.data.gsm,
        });
      } catch (error) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .put(`http://localhost:8081/api/v1/user/admin/update-agent/${username}`, formData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      })
      .then((res) => {
        toast.success("Félicitations ! Les informations agent a été modifié avec succès.");
        if (enabled) {
          setPdf(true)
        }
            ...
            {pdf ? (
              ...
              <>
                    ...
                    <div className="bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
                      <div className="md:grid md:grid-cols-3 md:gap-6">
                        <div className="md:col-span-1">
                          <h3 className="text-lg font-medium leading-6 text-gray-900">Modifier les informations de l'Agent</h3>
                          ...
                          <form onSubmit={handleSubmit}>
                            <div className="grid grid-cols-6 gap-6">
                              ...
                              <div className="col-span-6 sm:col-span-3">
                                ...
                                <span className='text-xs text-gray-400'>Écrire "NA" si vous ne souhaitez pas changer le mot de passe.</span>
                              </div>
                ...
                <div className="flex justify-start p-4">
                  <button
                    onClick={() => window.location.href = '/admin/agents/Index'}
                    className="ml-3 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500"
                  >
                    Retour
                  </button>
